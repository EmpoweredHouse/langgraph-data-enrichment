...
from enrichment_agent.state import Perspectives, GenerateAnalystsState, InterviewState, ResearchGraphState
...
async def generate_question(
    ...
    # Generate question
    system_message = SystemMessage(content=prompts.QUESTION_INSTRUCTIONS_PROMPT.format(goals=analyst.persona))
...
def generate_answer(
    ...
    # Answer question
    system_message = prompts.ANSWER_INSTRUCTIONS_PROMPT.format(goals=state.analyst.persona, context=state.context)
...
def write_section(
    ...
    section = raw_model.invoke([SystemMessage(content=system_message)] + [
...
def write_report(
    ...
    *,
    ...
    system_message = prompts.REPORT_WRITER_INSTRUCTIONS.format(topic=state.topic, context=formatted_str_sections)
...
def write_introduction(
    ...
    instructions = prompts.INTRO_CONCLUSTION_INSTRUCTIONS_PROMPT.format(topic=state.topic, formatted_str_sections=formatted_str_sections)
    intro = raw_model.invoke([instructions] + [HumanMessage(content=f"Write the report introduction")])
...
def write_conclusion(
    ...
    # Summarize the sections into a final report
    instructions = prompts.INTRO_CONCLUSTION_INSTRUCTIONS_PROMPT.format(topic=state.topic, formatted_str_sections=formatted_str_sections)
    conclusion = raw_model.invoke([instructions] + [HumanMessage(content=f"Write the report conclusion")])
